...
#' @export
dtedit <- function(session,
                   input, output, name, thedata,
    ...
    observeEvent(input[[paste0(name, '_insert')]], {
        ...
        row <- nrow(newdata) + 1
        newdata[row,] <- newdata[reference.row,]
        
        #-----------------------------------------------------------------------
        if (is.null(cam_config)) {
            corridor_row <- corridors[corridors$Description==input[[paste0(name, "_signalid")]],]
            detector_row <- det_config[
                det_config$Selector==input[[paste0(name, "_detector")]] &
                    as.character(det_config$SignalID)==as.character(corridor_row$SignalID),]
            
            newdata[row, "Zone"] <- corridor_row$Zone
            newdata[row, "Corridor"] <- corridor_row$Corridor
            newdata[row, "SignalID"] <- as.integer(as.character(corridor_row$SignalID))
            newdata[row, "CallPhase"] <- detector_row$CallPhase
            newdata[row, "Detector"] <- detector_row$Detector
            newdata[row, "Name"] <- as.character(corridor_row$Description)
            newdata[row, "ApproachDesc"] <- paste(detector_row$ApproachDesc, "Lane", detector_row$LaneNumber)
        } else {
            camera_row <- cam_config[cam_config$Description==input[[paste0(name, "_cameraid")]],]

            newdata[row, "Zone"] <- camera_row$Zone
            newdata[row, "Corridor"] <- camera_row$Corridor
            newdata[row, "CameraID"] <- as.character(camera_row$CameraID)
            newdata[row, "Name"] <- as.character(camera_row$Location)
        }
        #-----------------------------------------------------------------------
    ...
    addModal <- function(row, values) {
        ...
        #-----------------------------------------------------------------------
        if (is.null(cam_config)) {
            field0 <- selectInput(paste0(name, "_signalid"),
                                  label = "Signal",
                                  choices = corridors$Description,
                                  selected = NULL,
                                  width = select.width)
            field1 <- selectInput(paste0(name, "_detector"),
                                  label = "Detector",
                                  choices = NULL,
                                  selected = NULL,
                                  width = select.width)
            fields <- c( list(field0, field1), fields)
        } else {
            field0 <- selectInput(paste0(name, "_cameraid"),
                                  label = "Camera",
                                  choices = cam_config$Description,
                                  selected = NULL,
                                  width = select.width)
            fields <- c( list(field0), fields)
        }
        #-----------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------
    observeEvent(input[[paste0(name, "_signalid")]], {
        sid <- str_extract(input[[paste0(name, "_signalid")]], "^\\d+")
        if (!is.na(sid)) {
            
            print(det_config[det_config$SignalID==sid,])
            print(corridors[corridors$Description==input[[paste0(name, "_signalid")]],])

            updateSelectInput(
                session,
                inputId = paste0(name, "_detector"),
                choices = det_config[det_config$SignalID==sid,]$Selector)
        }
    })
    #---------------------------------------------------------------------------
    ...
    # output$exportData
    output[[paste0(name, '_exportData')]] <- downloadHandler('alerts.csv', content = function(file) {
        write_csv(result$thedata, file)
    })
    ...
    output[[name]] <- shiny::renderUI({
        shiny::div(
            ...
            downloadButton(paste0(name, '_exportData'), "Export"),
