...
yes_icon <- fa("check-circle", fill = GDOT_BLUE, height = "2em") %>% as.character()
no_icon <- fa("circle", fill = LIGHT_GRAY_BAR, height = "2em") %>% as.character()
...
get_cam_config <- function(object, bucket, corridors) {
    ...
    cams <- aws.s3::s3read_using(
        ...
        distinct() %>%
        left_join(corrs, by = c("SignalID"), suffix = c("_cams", "_corr")) %>%
        filter(!(is.na(Zone_cams) & is.na(Zone_corr)))

    cams1 <- cams %>%
        filter(is.na(Zone_corr)) %>%
        transmute(
            CameraID,
            Location,
            Zone_Group = Zone_Group_cams,
            Zone = Zone_cams,
            Corridor = Corridor_cams,
            Subcorridor,
            As_of_Date
        )
    ...
    cams2 <- cams %>%
        ...
        transmute(
            CameraID,
            Location,
            Zone_Group = Zone_Group_corr,
            Zone = Zone_corr,
            Corridor = Corridor_corr,
            Subcorridor,
            As_of_Date
        )
...
get_alerts_ben <- function() {
    ...
    detector_alerts <- alerts %>%
        ...
        mutate(Alert_Stk = glue("{Alert} ({streak} days)")) %>%
...
server <- function(input, output, session) {
    ...
    tsos_detector_alert_notes <- tbl(
        conn, "tsos_detector_alert_notes"
    ) %>%
        collect() %>%
        ...
        mutate(
            ...
            Cause.of.Malfunction = factor(
                ...
                levels = c(
                    "", "Resurface", "Hardware Failure", "POD", "Construction",
    ...
    observe({
        ...
        det_alert_notes_to_upload <- detector_alerts %>%
            ...
            filter(
                !is.na(LastModified),
                !uuid %in% dbReadTable(conn, "tsos_detector_alert_notes")$uuid
            ) %>%
            mutate(across(where(is.logical), as.integer))

        # Upload records not already in the database
        dbWriteTable(
            conn, "tsos_detector_alert_notes", det_alert_notes_to_upload,
            row.names = FALSE,
            overwrite = FALSE,
            append = TRUE
        )
    ...
    observe({
        ...
        cam_alert_notes_to_upload <- camera_alerts %>%
            ...
            filter(
                !is.na(LastModified),
                !uuid %in% dbReadTable(conn, "tsos_camera_alert_notes")$uuid
            ) %>%
            mutate(across(where(is.logical), as.integer))

        # Upload records not already in the database
        dbWriteTable(
            conn, "tsos_camera_alert_notes", cam_alert_notes_to_upload,
            row.names = FALSE,
            overwrite = FALSE,
            append = TRUE
        )
